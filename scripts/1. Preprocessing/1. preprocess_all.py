...
import sys
sys.path.append("/IMUPoser")

config = Config(experiment="preprocessing_run", project_root_dir="/IMUPoser")
...
def process_amass():
    ...
    for ds_name in amass_datasets:
        ...
        for npz_fname in tqdm(glob.glob(os.path.join(config.raw_amass_path, ds_name, '**/*_poses.npz'), recursive=True)):
